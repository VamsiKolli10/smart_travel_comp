...
function kmBetween(a,b){
  const R=6371; const dLat=deg2rad(b.lat-a.lat), dLng=deg2rad(b.lng-a.lng);
  const s1=Math.sin(dLat/2)**2 + Math.cos(deg2rad(a.lat))*Math.cos(deg2rad(b.lat))*Math.sin(dLng/2)**2;
  return 2*R*Math.atan2(Math.sqrt(s1),Math.sqrt(1-s1));
}

async function geocodeCity(city, language="en") {
  const { data } = await axios.get(NOMINATIM, {
    params: { q: city, format: "json", addressdetails: 1, limit: 1, "accept-language": language },
...
`;
  const { data } = await axios.post(OVERPASS, q, {
    headers: { "Content-Type": "text/plain; charset=UTF-8", "User-Agent": "smart-travel-companion/1.0" }
  });
...
function toResultItem(el, center) {
  ...
  const distanceKm = (center && lat && lng) ? Number(kmBetween(center, {lat, lng}).toFixed(2)) : null;
  const tags = el.tags || {};
